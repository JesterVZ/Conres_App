...
class ClaimMessagesPage extends StatefulWidget {
  String claim_id;
  String? page;
  ...
  @override
  State<StatefulWidget> createState() => _ClaimMessagesPage();
}

class _ClaimMessagesPage extends State<ClaimMessagesPage> {
  ...
  void _send() {
    profileBloc!
        .sendMessage(widget.claim_id, controller.text, widget.statusName!, files);
  }
  void _createPhoto(){
  ...
  void pagination() {
    if (scrollController.position.pixels ==
      ...
      setState(() {
        page++;
        profileBloc!.getMessages(widget.claim_id, page.toString(), lastMessageId!);
      });
    }
  }
                      ...
                      Container( 
                          ...
                          child: Padding(
                              ...
                              child: HeaderNotification(
                                  text: "Заявление № ${widget.claim_id}", canGoBack: true))),
  ...
  _listener(BuildContext context, ProfileState state) {
    ...
    if(page == 1){
      messageList = state.ticketFullInfo!.messages!;
        profileBloc!.readMessage(widget.claim_id, lastMessageId!);
        for (int i = 0; i < state.ticketFullInfo!.messages!.length; i++) {
          messageList[i].isOwn =
              state.ticketFullInfo!.messages![i].user_id != widget.claim_id ? false : true;
        }
    }
    ...
    if (state.sendMessageData != null) {
      MessageSend message = MessageSend(
          ...
          data: Data(
              user_type: "lk",
              user_id: int.parse(widget.claim_id),
              files: state.sendMessageData!['ticket_message_files'],
              ticket_id: int.parse(widget.claim_id),
              ticket_info: [
                TicketInfo(
          ...
          to_id: int.parse(widget.claim_id));
      ...
      if(state.sendMessageData!['ticket_info'][0]['files'] != null){
        messageFiles.clear();
        files = null;
        profileBloc!.getMessages(widget.claim_id, widget.page!, lastMessageId!);
      }
  ...
  void didChangeDependencies() {
    ...
    profileBloc!.getMessages(widget.claim_id, widget.page!, lastMessageId != null ? lastMessageId! : "1");
